...
void GLBuilder::clearScene()
{
    Controller::getInstance()->getScene()->objects.clear();
    emit newScene();
}

void GLBuilder::addTexture() {
    QString fileName = QFileDialog::getOpenFileName();
    if (!fileName.isNull()) {
        for (auto obj : Controller::getInstance()->getScene()->objects) {
            obj->setTexture(make_shared<QOpenGLTexture>(QImage(fileName).mirrored()));
            emit newObj(obj);
        }
    }
